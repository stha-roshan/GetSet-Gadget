...
async function fetchSimilarProducts(categoryId, currentProductId) {
  try {
    ...
    const cleanProductId = String(currentProductId).trim();

    const response = await fetch(
      `${API_BASE_URL}/product/similar?categoryId=${cleanCategoryId}&excludeId=${cleanProductId}&limit=3`,
    );
    ...
    if (result.success && result.data) {
      // console.log("Fetched similar products:", result.data);
      return result.data;
    } else {
...
function displayProductDetails(product) {
  ...
  const categoryValue = document.querySelector(
    ".product-meta .meta-item:nth-child(1) .meta-value",
  );
  categoryValue.textContent = product.category.name || "N/A";

  const brandValue = document.querySelector(
    ".product-meta .meta-item:nth-child(2) .meta-value",
  );
...
function displaySimilarProducts(products) {
  ...
  if (products.length === 0) {
    similarProductsGrid.innerHTML =
      '<p style="text-align: center; color: #6b7280; grid-column: 1/-1;">No similar products found</p>';
    return;
  }
...
async function initProductDetailPage() {
  ...
  try {
    ...
    const similarProducts = await fetchSimilarProducts(
      currentProductDetails.category._id,
      productId,
    );
