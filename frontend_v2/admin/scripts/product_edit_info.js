...
async function fetchAndPopulateProduct() {
    ...
    try {
        ...
        document.getElementById('productTotalSales').value = product.totalSales || 0;

        // Update character counters
        document.getElementById('nameCount').textContent = (product.name || '').length;
        document.getElementById('descCount').textContent = (product.description || '').length;

        // Set category and brand after dropdowns are populated
        ...
        // Show existing image as preview (without requiring re-upload)
        if (product.image) {
            ...
            previewContainer.innerHTML = `
                <div style="text-align: center;">
                    ...
                    <div class="image-info">📄 Current product image</div>
                    <div class="image-actions">
                        <button type="button" class="btn-icon" onclick="changeImage()">Change Image</button>
                    </div>
                </div>
            `;
...
document.getElementById("productForm").addEventListener("submit", async (e) => {
    ...
    if (!productId) {
        showToast('Product ID missing. Cannot update.', 'error');
        return;
    }

    const submitBtn = document.getElementById("submitBtn");

    // Image is optional on edit — only validate if no existing image shown
    const previewContainer = document.getElementById('previewContainer');
    const hasExistingImage = previewContainer.innerHTML.trim() !== '';
    ...
    if (totalSalesField.value.trim()) {
        if (!validateField(totalSalesField)) allValid = false;
    }
    ...
    if (!categoryValue) { showToast("Please select a category", "error"); allValid = false; }
    if (!brandValue)    { showToast("Please select a brand", "error");    allValid = false; }
    if (!allValid)      { showToast("Please fill all required fields correctly", "error"); return; }
    ...
    // Only append image if admin chose a new one
    if (productImageFile) {
        formData.append("image", productImageFile);
    }

    const totalSales = totalSalesField.value.trim();
    if (totalSales) formData.append("totalSales", parseInt(totalSales));

    isSubmitting = true;
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<span class="loading-spinner"></span> Updating Product...';

    try {
        const response = await fetch(`${API_BASE_URL}/product/edit/${productId}`, {
            method: "PATCH",
            credentials: 'include',
            body: formData,
        });
